...
SFE_BMP180::SFE_BMP180(TWI_Master_t *mytwi,int address)
{
	double c3,c4,b1;
		...
		readInt(0xBE,MD))
	{
		...
		output_print(KNET,"AC1: %d\n\r",AC1);
		output_print(KNET,"AC2: %d\n\r",AC2);
		output_print(KNET,"AC3: %d\n\r",AC3);
		output_print(KNET,"AC4: %d\n\r",AC4);
		output_print(KNET,"AC5: %d\n\r",AC5);
		output_print(KNET,"AC6: %d\n\r",AC6);
...
// value: external variable to store data (function modifies value)
{
	...
	if ( TWI_MasterWriteRead(twi,twi_address,&temp,1,2) )
	{
		while (twi->status != TWIM_STATUS_READY) {
			/* Wait until transaction is complete. */
		}
		value = (int16_t)(((twi->readData[0])<<8)|(twi->readData[1]));
		return true;
	} 
	else
		return false;
...
// value: external variable to store data (function modifies value)
{
	...
	if ( TWI_MasterWriteRead(twi,twi_address,&temp,1,2) )
	{
		...
		value = (uint16_t)(((twi->readData[0])<<8)|(twi->readData[1]));
		return true;
	}
	else
	return false;
...
// length: number of bytes to read
{
	...
	temp = values[0];
	if ( TWI_MasterWriteRead(twi,twi_address,&temp,1,length) )
	{
		while (twi->status != TWIM_STATUS_READY) {
			/* Wait until transaction is complete. */
		}
...
{
	if ( TWI_MasterWrite(twi,twi_address,values,length) )
	{
		while (twi->status != TWIM_STATUS_READY) {
			/* Wait until transaction is complete. */
		}
...
double SFE_BMP180::measure_pressure(double sealevel)
{
double T,P;
	delay_ms(this->startTemperature());
	if (this->getTemperature(T))
	{
		delay_ms(this->startPressure(2));
		if (this->getPressure(P,T))
		{
			if (sealevel>0)
				return(this->sealevel(P,sealevel));
			else
				return(P);			
		}
		else
...
void SFE_BMP180::delay_ms(int16_t i)
{
	int j;
	for (j=0;j<i;j++)
	{
